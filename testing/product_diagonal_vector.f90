...
A = new_matrix( reshape( [real(rb) :: 1,2,3,4 ], [1,4] ) )
B = new_matrix( [real(rb) :: 1,2,3,4 ] )
...
do while (.true.)
  ...
  C = new_matrix( reshape( [real(rb) :: 1,2,3,4,5,6,7,8 ], [1,4] ) )*(.d.new_matrix( [real(rb) :: 1,2,3,4 ] ))
  print*
  call print_matrix( C, "F6.1" )

  write(*,'(/,"Testing product of a temporary matrix with a permanent dmatrix...")')
  C = new_matrix( reshape( [real(rb) :: 1,2,3,4,5,6,7,8 ], [1,4] ) )*D
  print*
  call print_matrix( C, "F6.1" )
  ...
  C = (.d.new_matrix( [real(rb) :: 1,2,3,4 ] ))*new_matrix( reshape( [real(rb) :: 1,2,3,4,5,6,7,8 ], [4,1] ) )
  print*
  call print_matrix( C, "F6.1" )

  write(*,'(/,"Testing product of a permanent dmatrix with a temporary matrix...")')
  C = D*new_matrix( reshape( [real(rb) :: 1,2,3,4,5,6,7,8 ], [4,1] ) )
  print*
  call print_matrix( C, "F6.1" )
